...
public interface DeliveryPointGroupRepository extends CrudRepository<DeliveryPointGroup, Integer> {
    ...
    String FIND_ALL_STREET_TYPE_QUERY = "SELECT DISTINCT CONCAT(UCASE(MID(dpg.streetType,1,1)),LCASE(MID(dpg.streetType,2))) as streetType "
            + "FROM DeliveryPointGroup dpg WHERE dpg.streetType != '' ORDER BY streetType";
